...
namespace Binance.API.Csharp.Client
{
    public class ApiClient : ApiClientAbstract, IApiClient
    {
        ...
        public void ConnectToUserDataWebSocket(string parameters, MessageHandler<Models.WebSocket.AccountUpdatedMessage> accountHandler, MessageHandler<Models.WebSocket.OrderOrTradeUpdatedMessage> tradeHandler, MessageHandler<Models.WebSocket.OrderOrTradeUpdatedMessage> orderHandler)
        {
            var finalEndpoint = _webSocketEndpoint + parameters;
            ...
            ws.OnMessage += (sender, e) =>
            {
                ...
                switch (eventData.e)
                {
                    case "outboundAccountInfo":
                        accountHandler(JsonConvert.DeserializeObject<Models.WebSocket.AccountUpdatedMessage>(e.Data));
                        break;
                    case "executionReport":
                        ...
                        if (isTrade)
                        {
                            tradeHandler(JsonConvert.DeserializeObject<Models.WebSocket.OrderOrTradeUpdatedMessage>(e.Data));
                        }
                        else
                        {
                            orderHandler(JsonConvert.DeserializeObject<Models.WebSocket.OrderOrTradeUpdatedMessage>(e.Data));
                        }
                        break;
                }
            };
